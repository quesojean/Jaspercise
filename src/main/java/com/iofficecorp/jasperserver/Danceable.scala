...
final private case class DomainBuilder(resourceXML: Elem, tableIds:Seq[String], fieldProps: Map[FieldProps, String]) extends Danceable[SchemaType] {
  ...
  override val scope: NamespaceBinding = Namespace.DOMAIN
...
/**
 * Builds an adhoc or jasper report's nested xml resource.  The ReportBuilder's bound genarated
 * class is JasperReport
 *
 * TODO: check other types of reports (right now only table, we need to do crosstab, etc)
 *
...
final private case class ReportBuilder(resourceXML: Elem, tableIds:Seq[String], fieldProps: Map[FieldProps, String]) extends Danceable[JasperReport] {
  ...
  override val scope: NamespaceBinding = Namespace.REPORT
